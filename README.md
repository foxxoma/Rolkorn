...
Теперь разделите состояние на 5 этапов и опишите, какие изменения происходят на каждом из них. Эти этапы привязаны к ячейкам состояния.  
...
Каждый раз, когда вы сталкиваетесь с обстоятельствами усугубляющими состояние, бросайте d20 + основная характеристика шкалы (если предусмотрено), против текущей сложности. При провале начисляется 1 очко состояния.  

**Ячейки состояния**
У вас есть 5 ячеек состояния. Каждая ячейка вмещает в себе определенное количество очков состояния. Установите их для своей шкалы (1, 5, 10...) - чем меньше значение, тем быстрее заполняется шкала.  
Как только ячейка достигает своего лимита:  
- её состояние применяются  
- сложность увеличивается на 2  
- очки начинают начисляться в следующую ячейку  

Когда происходит восстановление состояния: бросьте d4, вычтете выпавшее значение из текущей ячейки  
- если выпавшее значение больше значения ячейки, начните отнимать от предыдущей и отмените ее эффекты.  
- (опционально) для некоторых шкал, можно запретить восстанавливать уже заполненные ячейки.  
